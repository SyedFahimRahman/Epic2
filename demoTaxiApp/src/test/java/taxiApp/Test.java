...
class TaxiHiringAppTestsTest {
    ...
    void testAddToMap() {
        ...
        assertTrue(tests.getMap().isTaxiOnMap(reg)); // Verify that taxi is on the map
    }
    ...
    void testRemoveVehicle() {
        ...
        assertFalse(tests.getMap().isTaxiOnMap(reg)); // Verify that taxi is not on the map after removal
    }
    ...
    void testGetVehiclesInRange() {
        ...
        assertTrue(vehiclesInRange.contains(reg2), "Expected " + reg2 + " to be in range. taxiApp.Location: " + loc2);
        assertFalse(vehiclesInRange.contains(reg1), "Expected " + reg1 + " to be out of range. taxiApp.Location: " + loc1);
    }
}